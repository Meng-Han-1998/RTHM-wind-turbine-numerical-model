...
!..................................................................................................................................
      ...
      !Hm: Force the platform motion and velocity to be same as the input from AQWA
      !IF (CouplingAQWA) THEN
      !   x%QT(DOF_Sg)   = PtfmMotAQWA(1)     
      !   x%QT(DOF_Sw)   = PtfmMotAQWA(2)     
      !   x%QT(DOF_Hv)   = PtfmMotAQWA(3)     
      !   x%QT(DOF_R)    = PtfmMotAQWA(4)     
      !   x%QT(DOF_P)    = PtfmMotAQWA(5)     
      !   x%QT(DOF_Y)    = PtfmMotAQWA(6)     
      !   x%QDT(DOF_Sg)  = PtfmVelAQWA (1) 
      !   x%QDT(DOF_Sw)  = PtfmVelAQWA (2) 
      !   x%QDT(DOF_Hv)  = PtfmVelAQWA (3) 
      !   x%QDT(DOF_R)   = PtfmVelAQWA (4) 
      !   x%QDT(DOF_P)   = PtfmVelAQWA (5) 
      !   x%QDT(DOF_Y)   = PtfmVelAQWA (6) 
      !   
      !   m%QD2T(DOF_Sg ) = PtfmAcceAQWA(1)
      !   m%QD2T(DOF_Sw ) = PtfmAcceAQWA(2)
      !   m%QD2T(DOF_Hv ) = PtfmAcceAQWA(3)
      !   m%QD2T(DOF_R  ) = PtfmAcceAQWA(4)
      !   m%QD2T(DOF_P  ) = PtfmAcceAQWA(5)
      !   m%QD2T(DOF_Y) = PtfmAcceAQWA(6)
      !END IF
      !!End of the change 
...
!   ENDIF
   ...
   !Hm: Tower base loads. Convert the unit from kN to N, with repect to the platform reference point frame 在平台局部参考系下
   !IF (CouplingAQWA) THEN
     PtfmFrcAQWA(1) = m%AllOuts( TwrBsFxt) *1000.0 
     PtfmFrcAQWA(2) = m%AllOuts( TwrBsFyt) *1000.0  
     PtfmFrcAQWA(3) = m%AllOuts( TwrBsFzt) *1000.0 
     PtfmMomAQWA(1) = m%AllOuts( TwrBsMxt) *1000.0 
     PtfmMomAQWA(2) = m%AllOuts( TwrBsMyt) *1000.0  
     PtfmMomAQWA(3) = m%AllOuts( TwrBsMzt) *1000.0 

     PtfmMomAQWA_Yaw = ( MomX0Trb(2) - MomBNcRt(2) ) *1000.0 !hm.平台局部偏航力矩（减去塔顶偏航力矩）
   !END IF
   !End of the change
...
!Hm: Ignore solving for platform acceleration
   IF ( p%DOFs%NActvDOF > 0 ) THEN
      IF (CouplingAQWA ) THEN
        IF (p%DOFs%NActvDOF>6) THEN
            m%AugMat_factor = m%AugMat( p%DOFs%SrtPS( 7:p%DOFs%NActvDOF ), p%DOFs%SrtPSNAUG(7:p%DOFs%NActvDOF) )
            m%SolnVec       = m%AugMat( p%DOFs%SrtPS( 7:p%DOFs%NActvDOF ), p%DOFs%SrtPSNAUG(1+p%DOFs%NActvDOF) )   

            CALL LAPACK_getrf( M=p%DOFs%NActvDOF-6, N=p%DOFs%NActvDOF-6, A=m%AugMat_factor, IPIV=m%AugMat_pivot, ErrStat=ErrStat2, ErrMsg=ErrMsg2 )
               CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)   
               IF ( ErrStat >= AbortErrLev ) RETURN
           
            CALL LAPACK_getrs( TRANS='N',N=p%DOFs%NActvDOF-6, A=m%AugMat_factor,IPIV=m%AugMat_pivot, B=m%SolnVec, ErrStat=ErrStat2, ErrMsg=ErrMsg2)
        END IF
      ELSE
...
END IF
   ...
   dxdt%QDT = 0.0
   !Hm:Assign the acceleration obtained from solving AQWA to QDT
   IF (CouplingAQWA ) THEN
     DO I = 1,6 ! Loop through all active (enabled) DOFs
        dxdt%QDT(p%DOFs%SrtPS(I)) = PtfmAcceAQWA(I)    ! dxdt%QDT = m%QD2T
     ENDDO             ! I - All active (enabled) DOFs
     IF (p%DOFs%NActvDOF>6) THEN
         DO I = 7,p%DOFs%NActvDOF ! Loop through all active (enabled) DOFs
            dxdt%QDT(p%DOFs%SrtPS(I)) = m%SolnVec(I-6)    ! dxdt%QDT = m%QD2T
         ENDDO             ! I - All active (enabled) DOFs
     END IF
   ELSE
     DO I = 1,p%DOFs%NActvDOF ! Loop through all active (enabled) DOFs
...
!Hm: Force the platform motion and velocity to be same as the input from AQWA
      ...
      END IF
! End of the change
! 对应FAST_V7 FAST.f90 8873行      
      ! find xdot at t FIRST
...
!..................................................................................................................................
    ...
    !Hm: Force the platform motion and velocity to be same as the input from AQWA，FAST 91113行
      IF (CouplingAQWA) THEN
        x%qt(DOF_Sg)   = PtfmMotAQWA(1)     
        x%qt(DOF_Sw)   = PtfmMotAQWA(2)     
        x%qt(DOF_Hv)   = PtfmMotAQWA(3)     
    ...
    ! End of the change            
      ...
      if (n .le. 2) then !Hm: 对应FAST_9192行
...
!..................................................................................................................................
      ...
      ErrMsg  = "" 
         ...
         ! predict:!Hm:FAST_V7 FAST.f90 9056行
...
SUBROUTINE FixHSSBrTq ( Integrator, p, x, OtherState, m, ErrStat, ErrMsg )
   ...
   IF (CouplingAQWA) THEN
     DO I = 1,6 ! Loop through all active (enabled) DOFs
        ...
        !      later if necessary
        RqdFrcGeAz = RqdFrcGeAz + m%OgnlGeAzRo(p%DOFs%SrtPS(I))*PtfmAcceAQWA(I)!0.0  ! {Fb}=[Cba]{Qa}+[Cbb]{Qb}
     ENDDO             ! I - All active (enabled) DOFs
     IF (p%DOFs%NActvDOF>6) THEN
         DO I = 7,p%DOFs%NActvDOF ! Loop through all active (enabled) DOFs
            ! bjj: use m%SolnVec(I) instead of m%QD2T(p%DOFs%SrtPS(I)) here; then update m%QD2T(p%DOFs%SrtPS(I))
            !      later if necessary
            RqdFrcGeAz = RqdFrcGeAz + m%OgnlGeAzRo(p%DOFs%SrtPS(I))*m%SolnVec(I-6)  ! {Fb}=[Cba]{Qa}+[Cbb]{Qb}
         ENDDO             ! I - All active (enabled) DOFs
     END IF
   ELSE
     DO I = 1,p%DOFs%NActvDOF ! Loop through all active (enabled) DOFs
   ...
   ELSE
      ...
      IF (CouplingAQWA) THEN
        DO I = 1,6 ! Loop through all active (enabled) DOFs
           m%QD2T(p%DOFs%SrtPS(I)) = PtfmAcceAQWA(I)!0.0
        ENDDO             ! I - All active (enabled) DOFs
        IF (p%DOFs%NActvDOF>6) THEN
            DO I = 7,p%DOFs%NActvDOF ! Loop through all active (enabled) DOFs
               m%QD2T(p%DOFs%SrtPS(I)) = m%SolnVec(I-6)
            ENDDO             ! I - All active (enabled) DOFs
        END IF
      ELSE
        DO I = 1,p%DOFs%NActvDOF ! Loop through all active (enabled) DOFs
